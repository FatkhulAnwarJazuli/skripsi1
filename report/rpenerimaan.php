...
$tgl = strftime("%e %B %Y"); // Tanggal saat ini di Banjarbaru adalah 27 Juni 2025
...
<head>
    ...
    <style>
        ...
        /* Sembunyikan kolom ID Penerimaan */
        .hide-column {
            display: none;
        }
    ...
    <table border="0" class="header-table">
        <tr>
            ...
            <td class="align-center">
                ...
                <hr class="line">
                <h3 style="text-align: center;">LAPORAN PENERIMAAN BARANG</h3>
            </td>
        </tr>
    </table>
    ...
    <div class="table-responsive">
        <table id="example1" class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>No</th>
                    <th class="hide-column">ID Penerimaan</th> <th>Tanggal Pemesanan</th>
                    <th>Nama Barang</th>
            ...
            <tbody>
                <?php
                // Query untuk mengambil semua data penerimaan dengan JOIN ke tabel pemesanan, pengajuan dan pegawai
                $query = "
                    SELECT
                        p.id_penerimaan,
                        p.tanggal_terima,
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    while ($data = mysqli_fetch_array($result)) {
                        ...
                        <tr>
                            ...
                            <td class="text-center hide-column"><?= htmlspecialchars($data['id_penerimaan']); ?></td> <td><?= htmlspecialchars($data['tanggal_pemesanan'] ? $data['tanggal_pemesanan'] : 'N/A'); ?></td>
                            <td><?= htmlspecialchars($data['nama_barang'] ? $data['nama_barang'] : 'N/A'); ?></td>
                            <td><?= htmlspecialchars($data['tanggal_terima']); ?></td>
                            <td><?= htmlspecialchars($data['kondisi_barang']); ?></td>
                            <td><?= htmlspecialchars($data['nama_pegawai'] ? $data['nama_pegawai'] : 'N/A'); ?></td>
                        </tr>
                        <?php
                    }
                } else {
                    // Sesuaikan colspan menjadi 6 (7 kolom total, dikurangi 1 yang disembunyikan)
                    ?>
                    <tr>
                        <td colspan="6" class="text-center">
                            <?php
                            if (!$result) {
                                echo "Gagal mengambil data penerimaan dari database: " . mysqli_error($conn) . ".";
                            } else {
