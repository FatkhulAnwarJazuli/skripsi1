...
// Hitung total data untuk paginasi dari tabel `barang_rusak`
// Note: We count from barang_rusak directly, as that's the primary table for this view.
...
if ($total_data_query) {
    ...
    error_log("Error counting total barang rusak: " . mysqli_error($conn)); // Log error
}
$total_pages = ceil($total_data / $limit);
$start_from = ($page - 1) * $limit;

// Query untuk mengambil data barang rusak dengan JOIN ke tabel 'barang'
// untuk mendapatkan nama barang dan merknya.
$query = "SELECT br.id_rusak, b.namabarang, b.merk, br.jumlah, br.kondisi, br.foto, br.status 
          FROM barang_rusak br
          JOIN barang b ON br.id_barang = b.id_barang
          ORDER BY br.id_rusak DESC LIMIT $start_from, $limit";
$result = mysqli_query($conn, $query);

// Cek apakah query berhasil
if (!$result) {
    die("Query data barang rusak gagal: " . mysqli_error($conn));
}
?>
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <h6 class="m-0 font-weight-bold text-primary">Laporan Data Barang Rusak</h6>
            <form action="report/rbarangrusak.php" method="POST" target="_blank">
                ...
                <table class="table table-bordered" id="myTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            ?>
                            <tr>
                                <td colspan="6" class="text-center">Tidak ada data barang rusak ditemukan.</td>
                            </tr>
                            <?php
                        }
                        ?>
    ...
    function filterTable() {
        ...
        for (i = 0; i < tr.length; i++) {
            ...
            if (tr[i].parentNode.tagName === 'TBODY') {
                ...
                // Loop melalui semua kolom yang relevan (Nama Barang Rusak (1), Merk (2), Jumlah (3), Kondisi (4), Status (5))
                for (j = 1; j < tr[i].cells.length; j++) { // Kolom 1 sampai 5
                    td = tr[i].cells[j];
                    if (td) {
    ...
    document.getElementById("searchInput").addEventListener("input", function() {
        ...
        debounceTimer = setTimeout(() => {
            if (inputVal.length >= 2) {
                // Pastikan Anda memiliki file 'barangrusak_suggestions.php'
