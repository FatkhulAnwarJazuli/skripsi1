...
// --- Ambil ID Barang untuk Dicetak ---
$id_barang_to_print = null;
if (isset($_POST['id_barang_to_print'])) {
    // Sanitasi input untuk mencegah XSS
    $id_barang_to_print = htmlspecialchars($_POST['id_barang_to_print']);
} else {
    // Hentikan eksekusi jika ID barang tidak ditemukan, karena ini halaman cetak spesifik
    die("ID Barang tidak ditemukan. Tidak dapat mencetak laporan.");
}
...
<html lang="id"> <head>
    ...
    <meta name="description" content="Halaman Cetak Laporan Data Barang Inventaris">
...
<body>
    <div class="kop-surat">
        ...
        <hr class="line">
        <h3 style="text-align: center; margin-top: 20px;">LAPORAN DATA BARANG</h3>
    </div>
    ...
    <table id="printTable" class="table table-bordered table-striped">
        ...
        <tbody>
            <?php
            // Mengambil data barang berdasarkan ID yang diterima
            $query = "SELECT namabarang, merk, jumlah FROM barang WHERE id_barang = ?";
            $stmt = mysqli_prepare($conn, $query);

            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "i", $id_barang_to_print); // "i" untuk integer
            ...
            <?php
                    ...
                    echo '<tr><td colspan="4" class="text-center">Data barang tidak ditemukan untuk ID ini.</td></tr>';
                }
                mysqli_stmt_close($stmt); // Tutup statement
            } else {
                echo '<tr><td colspan="4" class="text-center">Error saat menyiapkan query: ' . mysqli_error($conn) . '</td></tr>';
            }
            ?>
        </tbody>
    </table>
